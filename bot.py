...
if len(sys.argv) > 1:
    ...
    if command == '/start':
        ...
        if not apps:
            send_message("📊 هنوز داده‌ای نیست.")
            sys.exit()
        total = len(apps)
        ...
        rate = round(((interview + hired) / total) * 100, 1) if total > 0 else 0
        report = f"📊 <b>آمار KAVIAN NEXUS</b>\n"
        report += f"🎯 کل: {total}\n"
        report += f"📤 ارسال شده: {applied}\n"
        report += f"🎤 مصاحبه: {interview}\n"
        report += f"🎉 استخدام: {hired}\n"
        report += f"📈 نرخ پاسخ: {rate}٪"
        send_message(report)
        sys.exit()
        ...
        try:
            ...
            if 0 <= num < len(apps):
                ...
                save_memory(memory)
                send_message(f"✅ {apps[num]['title']}")
        except:
            send_message("⚠️ /applied 1")
        sys.exit()
